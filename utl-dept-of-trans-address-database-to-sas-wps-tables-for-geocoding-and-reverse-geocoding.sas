...
/*                   _     _                  _       _
___ ___  _ __ ___ | |__ (_)_ __   ___    __| | ___ | |_
/ __/ _ \| `_ ` _ \| `_ \| | `_ \ / _ \  / _` |/ _ \| __|
| (_| (_) | | | | | | |_) | | | | |  __/ | (_| | (_) | |_
\___\___/|_| |_| |_|_.__/|_|_| |_|\___|  \__,_|\___/ \__|
...
libname opnfix "m:/opnfix";

data finopn(compress=char);
retain from 'O';
label from="Openaddress database";
length zip4 $4 from $1;
set OPNFIX.ADR_010OPNSTAFIN(drop=zipstate);
zip4=zipcode;
drop zipcode;
lon=round(lon,.00001);
lat=round(lat,.00001);
...
data finNad;
retain from 'D';
label from="D-Dept of Transportation";
length adr $96 from $1;
set nad.adr_010nadall(drop=state rename=(avglon=lon avglat=lat)) ;
format lat lon 11.6;
lon=round(lon,.00001);
lat=round(lat,.00001);
run;quit;
...
data finOpnNad(compress=char);;
set finopn finNad;
run;quit;

proc sort data=finOpnNad  out=finOpnNadSrt noequals nodupkey;
by adr lon lat;
run;quit;

proc summary

proc summary data=finOpnNadSrt nway;
by  adr;
var lon lat;
output out=sd1.adr_010opnnadavg (compress=char drop=_freq_ _type_) mean=avglat avglon;
run;quit;
       ) ;
...
libname dot "g:/dept_of_trans";
data adr_010opnnadavg(Compress=char);
set dot.adr_010nadall_dot_adr_lat_lon (obs=10);
format lat lon 11.6;
avglon=round(lon,.00001);
avglat=round(lat,.00001);
drop state;
run;quit;


proc sort data=finNad  out=finNadSrtx noequals nodupkey;
by adr lon lat;
run;quit;

proc summary

proc summary data=finNadSrtx nway;
by  adr;
var lon lat;
output out=sd1.adr_010nadavg (compress=char drop=_freq_ _type_) mean=avglat avglon;
run;quit;

libname dot "g:/dot";

adr_010nadall_dot_adr_lat_lon_csv


data _null_;
%let _EFIERR_ = 0; /* set the ERROR detection macro variable */
%let _EFIREC_ = 0;     /* clear export record count macro variable */
file 'g:/dot/adr010nadavg.csv' delimiter=',' DSD DROPOVER lrecl=32767;
...
set  SD1.ADR_010NADAVG   end=EFIEOD;
   format ADR $96. ;
   format AVGLAT 11.6 ;
   format AVGLON 11.6 ;
 do;
 ...
 * after download and unzip;
 ...
 data wps_table;
   infile 'g:/dot/adr010nadavg.csv' firstobs=2 delimiter=',' DSD lrecl=32767;
   informat adr $96. avglat avglon 11.6;
   input adr avglat avglon;
   *if _n_=10 then stop;
 run;quit;
 ...
 %let _EFIERR_ = 0; /* set the ERROR detection macro variable */
 %let _EFIREC_ = 0;     /* clear export record count macro variable */
 file 'g:/dept_of_trans/adr010nadavg.csv' delimiter=',' DSD DROPOVER lrecl=32767;
 if _n_ = 1 then        /* write column names or labels */
  do;
    put
       "ADR"
